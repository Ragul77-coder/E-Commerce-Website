...
function triggerLogin() {
  if (isDemoMode) {
    // Setup mock user
    const dummyUser = {
      fullName: "Ragul Coder",
      imageUrl: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=150&h=150&q=80",
      email: "ragul@tapacademy.com"
    };
    localStorage.setItem('shop_mock_user', JSON.stringify(dummyUser));
    mockUser = dummyUser;
...
function setupSharedUI() {
  ...
  // 3. Demo Notification banner
  if (isDemoMode) {
    const wrapper = document.querySelector('.main-wrapper');
    const existingDemoBanner = document.querySelector('.demo-notification-banner');
    if (wrapper && !existingDemoBanner) {
      const demoBanner = document.createElement('div');
      demoBanner.className = 'demo-notification-banner';
      demoBanner.innerHTML = `
        <span>⚙️ Running in <strong>Demo Mode</strong>. You can test checkout with standard mock accounts.</span>
        <button id="set-clerk-btn">Configure Clerk Auth</button>
      `;
      wrapper.prepend(demoBanner);

      document.getElementById('set-clerk-btn')?.addEventListener('click', () => {
        const key = prompt("Enter your Clerk Publishable Key (pk_test_...):");
        if (key && key.trim() !== "") {
          // Temporarily store in localStorage to test active Clerk keys
          window.CONFIG = window.CONFIG || {};
          window.CONFIG.CLERK_PUBLISHABLE_KEY = key;
          localStorage.setItem('temp_clerk_key', key);
          location.reload();
        }
      });
    }
  ...
  // Retrieve temporary clerk key if available
  const tempKey = localStorage.getItem('temp_clerk_key');
  if (tempKey && window.CONFIG) {
    window.CONFIG.CLERK_PUBLISHABLE_KEY = tempKey;
  }
}
